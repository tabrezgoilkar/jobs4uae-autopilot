...
### Phase 7 — GCC Scanning, Tier 1
- Config-driven scanners by keyword/country/city → normalized listings into the app → one-click "Evaluate".
- **Acceptance:** Run a search → real listings returned and stored → evaluate one end-to-end.
- **Outcome (2026-06-22): Bayt + Naukrigulf MOVED TO ROADMAP** (decision: "non-working portal → remove from phase, add to roadmap"). Their cheerio-on-headless-fetch approach is Cloudflare-blocked and was never live-verified; the board files + tests were removed from the active codebase. The rework (real headed browser + XHR interception) lives in `docs/superpowers/plans/2026-06-23-NEXT-naukrigulf-bayt-scanner-rework.md` and is now a Phase 8 item.
- **Phase 7b — Indeed (DONE, live-verified 2026-06-22):** `server/scanner/boards/indeed.js` is the **first real working board**. Indeed embeds its job cards as JSON in the initial HTML (`window.mosaic.providerData["mosaic-provider-jobcards"]`), so the parser balance-extracts that blob — no XHR interception needed. Per-country regional domains (ae/sa/qa/kw/bh/om `.indeed.com`); clean `…/viewjob?jk=` URLs; captures title/company/location/salary/posted. **Anti-bot reality (verified):** Indeed sits behind Cloudflare — headless Playwright → "Blocked", node `fetch`/UA → 403 "Security Check". The working path is **headed Chromium + light anti-automation hardening** (real UA, `--disable-blink-features=AutomationControlled`, hide `navigator.webdriver`) → full results (15/scan live-confirmed for UAE & Qatar). `fetchHtml` is now headed-by-default with block-page detection + graceful failure. Datacenter IPs get rate-flagged after repeated hits; the user's residential IP is the reliable environment (matches the headed-browser posture below). Board carries `status: 'verified'`.
...
- **Robust scanning method = real headed browser + intercept the site's own JSON (anonymous, account-safe).** Phase 7 failed because it used *headless fetch + cheerio*; raw `curl`/headless are fingerprinted and blocked by Cloudflare instantly (verified live: `curl` to Naukrigulf's `spapi` returns HTTP 000). The fix is a **real, headed Playwright Chromium** running on the **user's own machine + residential IP** (a genuine browser fingerprint that passes Cloudflare like any human visit), navigating the **public** search page (no login), letting the page's JS resolve, and **intercepting the site's own XHR JSON response** (e.g. the `spapi` search call the page itself makes) via `page.on('response')` — capturing structured data without forging the Cloudflare-protected request. This is **anonymous → ~zero account-ban risk** (see account-safety posture below).
- **Why we can build this when commercial scrapers can too:** Apify-style scrapers (e.g. `easyapi/naukrigulf-jobs-scraper`, `k1ra/naukri-jobs-scraper`) get past Cloudflare with **rotating residential proxies + anti-detect cloud browsers** — infrastructure, not cleverer selectors. A local free tool can't replicate a proxy farm, but it doesn't need to: the user's **own real browser + real home IP** is the legitimate equivalent. Verification must happen **on the user's machine** (this dev sandbox can't reach these sites). An occasional Cloudflare "verify you're human" interstitial is resolved once by the user in the visible window.
- **Account-safety posture (a banned real account is worse than a failed scan).** **Scanning stays anonymous (no login)** → no account to ban. Authentication is reserved for **applying** (you must be logged in to submit), where the only automated step is *filling* fields at human pace and **the user clicks Submit** — normal account behaviour, not bulk automation. **LinkedIn is fully manual/assistive** for both scan and apply (the app reads what's already on the user's screen; it never programmatically drives LinkedIn), because LinkedIn aggressively bans automation. Everything stays **headed, human-paced, low-rate, on-demand**; what flags accounts is bot-like *behaviour* (speed/headless/volume), not being logged in. Before any board ever uses an authenticated session, show an explicit **ToS/ban-risk warning** + per-board opt-in.
- **ATS-hosted company career boards = the reliable "many boards" path (likely how `santifer/career-ops` gets 45–75+ working).** Career-ops's "portals" are mostly **company career pages**, which are hosted on ATSs (**Greenhouse, Lever, Ashby, Workday, SmartRecruiters**) that expose **public, un-protected JSON APIs** — e.g. `https://boards-api.greenhouse.io/v1/boards/<company>/jobs`, `https://api.lever.co/v0/postings/<company>?mode=json`, Ashby's posting API. **No Cloudflare, no anti-bot, no login, stable JSON.** Add an **ATS board type**: a config list of GCC-relevant employers (e.g. Careem, Talabat, Noon, banks, etc.) + which ATS each uses → reliable listings with zero scraping fragility. This is the highest-reliability, lowest-risk source and should likely be built **before** the Cloudflare aggregators.
