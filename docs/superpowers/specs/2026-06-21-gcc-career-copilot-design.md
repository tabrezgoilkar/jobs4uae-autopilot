...
- **Preferred fetch path = the connected, authenticated session (unifies with Auto-apply Connections).** The user connects/logs into a board once via the **Connections** flow (Phase 11) — a visible Playwright persistent browser; the user clears any login CAPTCHA themselves. Scanning then runs **as that authenticated, real-browser session** (its cookies + fingerprint), which is far more likely to pass Cloudflare/anti-bot than anonymous headless fetch, and can reach the logged-in JSON APIs. So the **persistent-session Connections manager is the shared foundation for both scanning (Phases 7–9) and applying (Phase 11)** — connect once, use for both. Honesty: it improves success a lot but doesn't *guarantee* bypass (anti-bot can still flag fast/automated navigation — keep it headed + human-paced + low-rate), and it stays ToS-gray (the user's own account, personal use).
- **Robust scanning method (revised after live testing — Phase 7's HTML selectors fail live).** Bayt/Naukrigulf are JS-rendered + Cloudflare-protected, so cheerio-on-`fetchHtml` returns nothing. Switch scanners to each board's **internal JSON search API** where one exists (e.g. Naukrigulf's `spapi` JSON search — uses **numeric country codes** like `country=3` and pagination params `xz=…`, the same pattern commercial scrapers such as Apify's `naukrigulf-jobs-scraper` drive). Fall back to a **real headed Playwright session** (`waitForSelector`/`networkidle`, light stealth) only where no JSON API exists. Keep it low-volume from the user's own machine. **Honesty:** board scraping / internal-API use is ToS-gray; keep it personal-use, low-rate, per-board verified before production. The Phase 7 Bayt/Naukrigulf boards are reworked to this approach as part of their verification.
