...
export function analyzeProfile(p: Profile): ProfileAnalysis {
  ...
  if ((p.summary?.trim().length ?? 0) < 120) suggestions.push({ title: 'Strengthen your summary', detail: 'Aim for 2–3 sentences covering your strongest, most relevant experience.', section: 'basics' });
  if ((p.skills?.length ?? 0) < 5) suggestions.push({ title: 'Add more skills', detail: 'List at least 5 relevant skills — they drive your job-match score.', section: 'basics' });
  if (p.experience?.some((x) => !x.description?.trim())) suggestions.push({ title: 'Add results to your roles', detail: 'Numbers beat adjectives — e.g. "cut checkout drop-off 18%".', section: 'experience' });
